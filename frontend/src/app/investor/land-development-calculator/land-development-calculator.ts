...
@Component({
  selector: 'app-investor-land-development-calculator',
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink, RouterLinkActive],
  templateUrl: './land-development-calculator.html',
  styleUrl: './land-development-calculator.scss'
})
export class InvestorLandDevelopmentCalculatorComponent implements OnInit {
  landAcres = 5;
  ...
  onLandAcresChange(): void {
    if (!this.builtSqftOverridden) this.builtSqft = this.landAcres * this.BUILT_SQFT_PER_ACRE;
    this.calculate();
  }

  onBuiltSqftChange(): void {
    this.builtSqftOverridden = this.builtSqft !== this.landAcres * this.BUILT_SQFT_PER_ACRE;
    this.calculate();
  }
  ...
  calculate(): void {
    ...
    const landCost = this.landCostMode === 'per_acre' ? this.landAcres * this.landCostValue : landSqft * this.landCostValue;
    const horizCost = this.horizMode === 'total' ? this.horizValue :
      this.horizMode === 'per_acre' ? this.landAcres * this.horizValue : this.builtSqft * this.horizValue;
    const vertCost = this.vertMode === 'total' ? this.vertValue : this.builtSqft * this.vertValue;
    ...
    this.result = { landSqft, landCost, horizCost, vertCost, totalCost, costPerBuiltSqft, far, landPct, horizPct, vertPct, grossAnnualRent, egi, noi, grossYield, netYield, stabilizedValue, devProfit, devMargin };
  }

  get landCostLabel(): string { return this.landCostMode === 'per_acre' ? '/ acre' : '/ sqft'; }
  get horizLabel(): string { return this.horizMode === 'total' ? 'Total' : this.horizMode === 'per_acre' ? '/ acre' : '/ built sqft'; }
  get vertLabel(): string { return this.vertMode === 'total' ? 'Total' : '/ built sqft'; }
  fmt(val: number): string { return '$' + Math.round(val).toLocaleString('en-US'); }
  fmtDec(val: number, d = 2): string { return val.toFixed(d); }
  fmtSqft(val: number): string { return Math.round(val).toLocaleString('en-US') + ' sqft'; }
